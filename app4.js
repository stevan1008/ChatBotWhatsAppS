...
/*       if(message.body === '!ping') {
    ...
    phoneNumber = message._data.from.split('@')[0];
    let user = null;
...
const chatbot = async function (user, phoneNumber, message) {
    let chatTree = await createChatTree();
            ...
            if (valTree) {
                if (eval(key + ".hasOwnProperty('validation')")) {
                    if (eval(eval(key + '.validation') + "('" + message._data.body.trim() + "')")) {
                        ...
                        if (eval(key + ".hasOwnProperty('function')")) {
                            eval(eval(key + '.function') + "('" + message._data.body.trim() + "')")

                            if (eval(key + ".hasOwnProperty('remakeTree')")) {
...
const createChatTree = function() {
    ...
    chatTree['6'] = {value: `La fecha y hora actual es ${time}`, function: 'getTime', remakeTree: true};
